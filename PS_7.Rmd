...
all_adv <- adv1 %>%
  mutate(rep_adv_results = 
           round((Rep - Dem) / all_weights, digits = 2) * 100, 
         dem_adv_results = 
           round((Dem - Rep) / all_weights, digits = 2) * 100)
all_adv <- all_adv %>%
...
results <- results %>%
  mutate(rep_adv_results = round((rep_votes - dem_votes) / 
           (dem_votes + rep_votes + other_votes), digits = 2) * 100, 
...
# calculate final weights (based on age)
advantage_age <- new_x %>%
  group_by(state_district, source) %>%
  mutate(all_weights = sum(final_weight)) %>%
  group_by(state_district, ager, response, source) %>%
  mutate(weights = sum(final_weight)) %>%
  select(state_district, state, weights, all_weights, ager, response) %>%
  distinct()
...
write_rds(all_adv_age, "ps_7/upshot_age.rds")
